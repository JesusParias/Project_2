...
int main(int argc, char** argv) {
    ...
    for(int draw=1; draw<=cardI; draw++){
        cout << fixed << setw(13) << draw<< ", ";
    }cout <<endl;
    for(int draw=1; draw<=cardI; draw++){
            num = (rand()%108)+1;
            if(deck[num]!=empty){
                deckB[num]=deck[num];
                deck[num]=empty;
                cardsU[draw]=deckB[num];
                //cout<<num <<endl<< "A "<< deck[num]<<endl<<"B "<< deckB[num]<<endl;
                cout << fixed << setw(13) <<cardsU[draw]<< ", ";
            }else if(deck[num]==empty){
                draw--;
                //cout << num<<endl;
            }
    }cout<<endl;
    ...
    for(int draw=1; draw<=cardI; draw++){
            ...
            if(deck[num]!=empty){
                deckB[num]=deck[num];
                deck[num]=empty;
                cardsAI[draw]=deckB[num];
                //cout<<num <<endl<< "A "<< deck[num]<<endl<<"B "<< deckB[num]<<endl;
                cout << fixed << setw(13) << cardsAI[draw]<< ", ";
            }else if(deck[num]==empty){
                draw--;
                //cout << num<<endl;
            }
    }cout << endl;
    for(int draw=1; draw<=1; draw++){
            num = (rand()%108)+1;
            if(deck[num]!=empty){
                deckB[num]=deck[num];
                deck[num]=empty;
